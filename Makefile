...
help: ## Shows this message.
	@echo "Asynchronous Python client for Meppel Afvalkalender."; \
	echo; \
	echo "Usage:"; \
	awk -F ':|##' '/^[^\t].+?:.*?##/ {\
...
lint-black: ## Run linting using black & blacken-docs.
	black --safe --target-version py35 meppel_afvalkalender tests examples; \
	blacken-docs --target-version py35

.PHONY: lint-flake8
lint-flake8: ## Run linting using flake8 (pycodestyle/pydocstyle).
	flake8 meppel_afvalkalender

.PHONY: lint-pylint
lint-pylint: ## Run linting using PyLint.
	pylint meppel_afvalkalender

.PHONY: lint-mypy
lint-mypy: ## Run linting using MyPy.
	mypy -p meppel_afvalkalender

.PHONY: test
test: ## Run tests quickly with the default Python.
	pytest --cov-report html --cov-report term --cov=meppel_afvalkalender .;
